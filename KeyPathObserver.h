 ...
 *
 * @note affecting the same value as the already affected value for a keyPath (self.prop = self.prop) will still trigger the block execution according to KVO.
...
/**
 * Remove all observed keypath from the object.
 * @note This is automatically called when the observed object (self) is deallocateed
 */
